...
const ROUTER_ERROR = "router参数错误";//开发环境

class User {
    constructor() {
        //node 8.9x以下版本不支持 a = async () => {} 语法
    ...
    async login(req, res) {  //登录
        ...
        try{
            ...
            assert(options.password, "密码不能为空");
            const address = await axios.get(`http://ip.taobao.com/service/getIpInfo.php?ip=${req.ip}`);
    ...
    async removeCollect(req, res, next) {
        if (req.body.collect_id) {
            try{
                await service.removeCollect({ collect_id: req.body.collect_id, user_id: req.session.user });
                await res.status(200).end();
            }catch(err){
                next(err);
            }
    ...
    async getCollectPosts(req, res, next) { //获取单个收藏夹所有收藏文章
        ...
        if (req.params.collect_id && req.query.page) {
            try{
                ...
                const { collect, user, collect_count, update_date } = await service.getCollectPosts({ skip, collect_id });

                const posts = await this.flatten(collect);

                await res.json({ posts, user, update_date, count: Math.ceil(collect_count / 10) });
            }catch(err){
                next(err);
            }
        } 
    ...
    async setFollowing(req, res, next) {    //关注某个用户
        if (req.body.author_id && req.body.action && req.session.user !== req.body.author_id ) {
            try{
