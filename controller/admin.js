...
class Admin{
    ...
    async getChart(req, res, next){
        try{
            ...
            chart.user.current_count = await this.setvalue(chart.user.date_arr[chart.user.date_arr.length-1]);
            chart.posts.current_count = await this.setvalue(chart.posts.date_arr[chart.posts.date_arr.length-1]);
    ...
    async getArticle(req, res, next){
        if(req.query.state !== undefined){
            try{
                const article = await service.getArticle(req.params.posts_id, Number(req.query.state));
    ...
    async agreeArticle(req, res, next){
        if(req.body.posts_id && req.body.state !== undefined && req.body.page){
            try{
                ...
                const p = await service.agreeArticle({ check_posts_id: posts_id, state: state, skip: skip });
                const table = await this.addIndex(p);
    ...
    async getLabel(req, res, next){
        if(req.query.page && req.query.type){
            try{
                ...
                const { label, count } = await service.getLabel({ type: req.query.type, skip: skip });
    ...
    async removeLabel(req, res, next){
        if(req.body.label_id && req.body.page){
            try{
                ...
                const { label, count } = await service.removeLabel({ label_id: req.body.label_id, skip: skip });
