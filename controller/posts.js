...
class Posts {
    ...
    async getUserPostsList(req, res, next){
        if(req.session.user && req.query.page && req.query.type === "all"){
            try{
                ...
                if(!req.session.page){
                    const count = await service.getFollowPosts({ user_id:user_id, get_count:true });
                    req.session.page = await this.setvalue(Math.ceil( count / 10 )); //如果没有关注任何标签，直接跳到第二个逻辑
                }

                if(req.session.page >= page){
                    const { pa, pb, limit } = await service.getUserPostsList({ user_id, skip, followtype });
                    const posts = await this.flatten(pa);
                    if(req.session.page === page) req.session.limit = await this.setvalue(limit); //到达尾部查询填补了几个文档，skip获取跳过
                    await res.json({ posts: posts.concat(pb) });
                }
                else{
                    const posts = await this.getNotFollowtypePosts(followtype, page - req.session.page, req.session.limit);
    ...
    async getHotPostsList(req, res, next) { //最热
    ...
    async getSearch(req, res, next){ //搜索
    ...
    async getLabel(req, res, next) {
        if(req.query.type && req.query.page || req.query.search && req.query.page){
            try{
                ...
                const label = await service.getLabel({ [key]: value, skip: skip, user_id: user_id });
    ...
    async createArticle(req, res, next) {  //创建和更新文章
    ...
    async removeArticle(req, res, next) {
        if (req.body.posts_id && req.body.page){
            try{
                ...
                const skip = this.page(page, 10);
                const { people, count } = await service.removeArticle({ posts_id, author_id, skip });
    ...
    async getComment(req, res, next) {
        if (req.query.posts_id && req.query.page && req.query.sort) {
            try{
                ...
                const { comment, count } = await service.getComment({ posts_id, skip, sort });
