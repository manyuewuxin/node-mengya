...
class Admin {
    ...
    async getChart(){
        ...
        for(let n = 0; n < length; n++){ //不要使用promise.all
            ...
            posts.date_arr[n] = await collection.posts.find({ $and: [ 
                { date:{ $gte: new Date(week[n]) } },
                { date:{ $lt: new Date(week[n+1]) } }
            ] }).count();

            user.date_arr[n] = await collection.user.find({ $and: [ 
                { date:{ $gte: new Date(week[n]) } }, 
                { date:{ $lt: new Date(week[n+1]) } } 
            ] }).count();
        }
    ...
    async agreeArticle({ check_posts_id, state, skip }){ //审核通过
        if(typeof check_posts_id == "string" && typeof state == "number" && typeof skip == "number"){

            const { posts_id, ...article } = await collection.checkposts.findOne({ _id: new ObjectId(check_posts_id) });

            if(posts_id===undefined){ //如果不是更新就创建他
                const { ops } = await collection.posts.insertOne(Object.assign({},article,{
                ...
                await collection.comment.insertOne({ posts_id: new ObjectId(ops[0]._id), comment_count: 0, comment: [] }); //创建文章所属子文档评论
                await collection.user.findOneAndUpdate({ _id: new ObjectId(ops[0].author_id) }, { $inc: { create_p_count: +1 }}); //作者创建文章+1
                await collection.dynamic.findOneAndUpdate({ user_id: new ObjectId(ops[0].author_id) }, { 
                    $push: { dynamic: { posts_id: new ObjectId(ops[0]._id), dynamic_type: 'create', date: new Date(article.date) } },
                    $inc: { dynamic_count: +1 } 
                });
                await collection.label.findOneAndUpdate({ type: article.type[0] }, { $inc: {article_count: +1 }}); //为该文档类型+1
            }
            else{ //是更新就覆盖掉posts的文档
                await collection.posts.findOneAndUpdate({ _id: new ObjectId(posts_id) }, { $set: Object.assign({}, article, {state: 2})});
            }
            await collection.checkposts.findOneAndDelete({ _id: new ObjectId(check_posts_id) }); //删除
            if(state === 2){
    ...
    async rejectArticle({ posts_id, state, skip }){ //拒绝通过文章
        if(typeof posts_id == "string" && typeof state == "number" && skip == "number"){
            if(state === 0){
                await collection.checkposts.findOneAndUpdate({ _id: new ObjectId(posts_id) }, { $set: { state: 1 } });
                return this.$checkpostsLookup({ state: 0 },skip);
            }
            else if(state === 2){
                const article = await collection.posts.findOne({ _id: new ObjectId(posts_id) }); //获取
                await collection.checkposts.insertOne(Object.assign({}, article, { posts_id: article._id, state: 1})); //插入检查集合
                return this.$postsLookup({},skip,10);
            }
        }
    ...
    async AdminRemoveArticle({ posts_id, state, skip }){ //管理员删除文章
        if(typeof posts_id == "string" && typeof state == "number"){
            if(state === 0 || state === 1){
                await collection.checkposts.findOneAndDelete({ _id: new ObjectId(posts_id) });
                return this.$checkpostsLookup({ state: 0 },skip);
            }
            else if(state === 2){
                const { value } = await collection.posts.findOneAndDelete({ _id: new ObjectId(posts_id) });
                await this.r_ArticleAssociatedDocument({ posts_id: posts_id, author_id: value.author_id });
                return this.$postsLookup({},skip,10);
            }
        }
    ...
    async getTableUser(skip){
        if(typeof skip == "number"){
            ...
            const user = await collection.user.find({}).sort({ date: -1 }).skip(skip).limit(10).project({ name:1, date:1, region:1 }).toArray();
            return { user, count };
        }
    ...
    async getTablePosts(skip){
        if(typeof skip == "number"){
            ...
            return { p: pa.concat(pb), count: checkposts_count + posts_count, limit: limit };   
        }
    ...
    $checkpostsLookup(query,skip){
        if(typeof query == "object" && typeof skip == "number"){
            return collection.checkposts.aggregate([{ $match: query }, { 
                $lookup: { 
                    from: "user", 
                ...
                { $skip: skip }, 
                { $limit: 10 },
                { $project: { author:1, title:1, html:1, image:1, state:1, date:1 } }
            ]).toArray();
        }
    ...
    $postsLookup(query,skip,limit){
        if(typeof query == "object" && typeof skip == "number" && typeof limit == "number"){
            return collection.posts.aggregate([{ $match: query }, { 
                $lookup: { 
                    from: "user", 
                    let: { author_id: "$author_id" }, 
                    pipeline: [
    ...
    async getLabel({ type, skip }){
        if(typeof type == "string" && typeof skip == "number"){
            ...
            const label = await collection.label.find(query).sort({ date: -1 }).skip(skip).limit(16).toArray();
    ...
    updateLabel({ label_id, ...options }){
        if(typeof options == "object" && typeof label_id == "string"){
            return collection.label.findOneAndUpdate({ _id: new ObjectId(label_id) },{ $set: options });
        }
