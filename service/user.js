...
class User {
  ...
  getUser(user_id = null) {
    if (user_id) {
      return collection.user.findOne({ _id: new ObjectId(user_id) }, { projection: { password: 0, salt: 0 } });
    }
    return null;
  }
  ...
  async login(options) {
    ...
    if (typeof options == "object") {
      ...
      if (user !== null) {
        ...
        if (test) {
          const { password, salt, ...account } = user;
          return account;
        } else {
  ...
  async register(options,address) { //注册
    if(typeof options == "object" && typeof address == "object") {
      ...
      options.like = 0;
      options.collect = 0;
      options.information = "这个人没有写介绍"; //默认介绍
      options.carrer = ''; //职业
      options.company = ''; //公司
      options.userlike = ''; //个人主页
      options.region = address.code === 0 ? address.data.region : "湖南省";
      var salt = await bcrypt.genSalt(10); //加密强度
      options.salt = await bcrypt.hash(options.password, salt); //获取盐加密返回哈希表，开发阶段明文保存明文密码

      var user = await collection.user.insertOne(options);
      await collection.dynamic.insertOne({ user_id: new ObjectId(user.ops[0]._id), dynamic_count: 0, dynamic: [] });
      await collection.message.insertOne({ user_id: new ObjectId(user.ops[0]._id), read_count: 0, message: [] });
      return user;
    }
  ...
  async getAuthor(options) {
    if(typeof options == "object") {
      ...
      if(bool) {
        author.posts = await collection.posts.find({ author_id: new ObjectId(options.user_id) }).sort({ title: -1 }).limit(3).project({ title: 1 }).toArray();
      }
      return author;
    }
  ...
  getCollectList(options) { //获取收藏夹列表，多文档
    if(typeof options == "object") {
       return collection.collect.find({ user_id: new ObjectId(options.user_id) }).sort({ date: -1 }).skip(options.skip).limit(10).toArray();
    }
  ...
  async getCollectPosts(options) { //获取某个收藏夹收藏的文章//使用$zip
    if(typeof options == "object") {
      ...
      const { update_date, user_id, collect_count } = await collection.collect.findOne(
        { _id: new ObjectId(options.collect_id) },
        { projection: { update_date: 1, user_id: 1, collect_count:1 } }
      );

      const user = await collection.user.findOne({ _id: new ObjectId(user_id) }, { projection: { name: 1, avatar: 1 } });

      const collect = await collection.collect.aggregate([{ $match: { _id: new ObjectId(options.collect_id) } },
  ...
  async updateCollect(options) {  //更新收藏夹
    if(typeof options == "object") {
      ...
      const dynamic = {
          add: { posts_id: new ObjectId(options.posts_id), dynamic_type: "collect", date: new Date() },
          remove: { posts_id: new ObjectId(options.posts_id), dynamic_type: "collect" }
      };

      await collection.collect.findOneAndUpdate({ _id: new ObjectId(options.collect_id) }, { 
          [this.type[action]]: { collect_article: new ObjectId(options.posts_id) },
          $set: { update_date: new Date() },
          $inc: { collect_count: this.compute[action] } 
      });

      await collection.user.findOneAndUpdate({ _id: new ObjectId(options.author_id) }, { $inc: { collect: this.compute[action] } });

      await collection.dynamic.findOneAndUpdate({ user_id: new ObjectId(options.user_id) }, {
  ...
  removeCollect(options) {
    if(typeof options == "object") {
      collection.collect.findOneAndDelete({ _id: new ObjectId(options.collect_id), user_id: new ObjectId(options.user_id) });
      return true;
    }
  ...
  async setAccount(options, user_id, method) {
      if(typeof options == "object" && user_id && method == "profile") {
          await collection.user.findOneAndUpdate({ _id: new ObjectId(user_id) }, { $set: options } );
          return true;
      } 
      else if(typeof options == "object" && user_id && method == "password") {
          let user = await collection.user.findOne( { _id: new ObjectId(user_id) }, { projection: { salt: 1 } });
          let test = await bcrypt.compare(options.password, user.salt);
          if(test) {
  ...
  async getMessage(options) { //获取消息
    if (typeof options == "object") {
      ...
      const { read_count } = await collection.message.findOne({ user_id: new ObjectId(options.user_id) }, { projection: { read_count: 1 } });
      const message = await collection.message.aggregate([{ $match: { user_id: new ObjectId(options.user_id) } },
          { $unwind: "$message" },
          ...
          { $limit: 20 },
          { $project: { posts: 1, author: 1, "message.message_type": 1, user_id: 1 } }
      ]).toArray();
  ...
  setMessage(user_id) { //更新消息
    if(user_id) {
      return collection.message.findOneAndUpdate({ user_id: new ObjectId(user_id) }, { $set: { read_count: 0 } });
  ...
  async peopledynamic(options) { //获取动态
    if(typeof options == "object") {
      ...
      const people = await collection.dynamic.aggregate([ { $match: { user_id: new ObjectId(options.user_id) } },
          { $unwind: "$dynamic" },
          ...
          { $project: { posts: 1, "dynamic.dynamic_type": 1, "dynamic.date": 1 } }
      ]).toArray();
  ...
  async peoplearticle(options) {
    if (typeof options == "object") {
      const count = await collection.user.findOne({ _id: new ObjectId(options.user_id) }, { projection: { create_p_count: 1 } });
      const people = await collection.posts.aggregate([{ $match: { author_id: new ObjectId(options.user_id) } }, { 
          $lookup: {
                from: "user", 
  ...
  async peoplecollect(options) {
        ...
        const count = await collection.collect.find({ user_id: new ObjectId(options.user_id)}).count();
  ...
  async getFollow(options) { 
    ...
    const count = await collection.user.findOne({ _id: new ObjectId(options.user_id) }, { projection: { [f_count]: 1 } });

    const people = await collection.user.aggregate([ { $match: { _id: new ObjectId(options.user_id) } },
        { $unwind: type },
        ...
        { $lookup: {
            ...
            pipeline: [
              { $match: { $expr: { $eq: ["$_id", "$$user_id"] } } },
              { $project: { name: 1, avatar: 1, create_p_count: 1, followers_count: 1, information: 1 } }
            ],
            as: "author"
          }
        },
  ...
  async setFollowing(options, action) {
    if(typeof options == "object" && action === "add" || action === "remove") {
      ...
      await collection.user.findOneAndUpdate({ _id: new ObjectId(options.user_id) }, { //添加关注的ID
          [this.type[action]]: { following: new ObjectId(options.author_id) },
          $inc: { following_count: this.compute[action] }
      });
      await collection.user.findOneAndUpdate({ _id: new ObjectId(options.author_id) }, { //被关注添加ID
          [this.type[action]]: { followers: new ObjectId(options.user_id) },
          $inc: { followers_count: this.compute[action] }
      });
      await collection.message.findOneAndUpdate({ user_id: new ObjectId(options.author_id) }, { //被关注发送消息
  ...
  async setFollowtype(options){
      if(typeof options == "object") {
          ...
          await collection.user.findOneAndUpdate({ _id: new ObjectId(options.user_id) }, {
              [this.type[action]]: { followtype: options.type }
          });

          await collection.label.findOneAndUpdate({ type: options.type },{ $inc: { followtype_count: this.compute[action] } });
          return true;
      }
